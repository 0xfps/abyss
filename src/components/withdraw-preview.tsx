...
import TinyMerkleTree, { CircomInputObject, generateDepositKey, getInputObjects, getLeafFromKey } from "@fifteenfigures/tiny-merkle-tree";
...
export function WithdrawPreview() {
    ...
    useEffect(function () {
        const { abyssAddress } = abyssConfig.testnetConfig.chainsConfig[pollChainId]
    ...
    async function generateProof() {
        ...
        let tree: TinyMerkleTree | undefined

        try {
            tree = new TinyMerkleTree(leaves)
        } catch {
            toaster({
                title: "Tree computation failed",
                description: "There's only one leaf in the tree. A tree needs at least, two leaves.",
        ...
        let inputObjects: CircomInputObject | undefined

        try {
            inputObjects = getInputObjects(withdrawalKey, leaf, secretKey, tree)
        } catch {
            toaster({
        ...
        try {
            ...
            if (proof) {
                ...
                toaster({
                    ...
                    description: "The merkle proof for your leaf could not be computed.",
                    icon: <BiSolidErrorCircle />
                })
    ...
    async function withdraw() {
        ...
        try {
            ...
            toaster({
                ...
                description: "Your withdrawal transaction failed because you declined the request.",
                icon: <BiSolidErrorCircle />
            })
